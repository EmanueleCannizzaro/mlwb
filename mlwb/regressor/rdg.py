...
solver = Select(title="Solver:", value="auto", options=["auto", "svd", "cholesky", "lsqr", "sparse_cg", "sag", "saga"])   
...
def update():
    ...
    clf = Ridge(fit_intercept = fit_intercept1, normalize = normalize1, copy_X = copy_X1)
    clf.fit(x_train_original,y_train_original)
    predictions=clf.predict(x_test_original)
    ...
    stats2.text = "<h3> Results: </h3> </br>"
    stats2.text += "Mean Squared Error: %.2f" % mean_squared_error(y_test_original, predictions) + '</br>'
    stats2.text += " Variance score: %.2f" % r2_score(y_test_original, predictions) + '</br>'
    stats2.text += " Cross Validation score: %.2f " % scores.mean()


controls = [features,kfold, fit_intercept, normalize, copy_X, solver]
