...
n_neighbors = Slider(start=1, end=10, value=5, step=1,title="No of neighbors")
...
def update():
    ...
    n_neighbors1 = int(n_neighbors.value)
    algorithm1 = algorithm.value
    clf = KNeighborsClassifier(n_neighbors = n_neighbors1, algorithm = algorithm1)
    clf.fit(x_train_original,y_train_original)
    predictions=clf.predict(x_test_original)
    print("Accuracy =", accuracy_score(y_test_original,predictions))
    y_score = clf.predict_proba(x_test_original)[:,1]
...
controls = [features, n_neighbors, algorithm]
